...
const AGENT_PROGRAM = new PublicKey("FDUE22UDZUMcGXeDRKJJbNX1GMh4CREWnziam8GkmerY");
const LLM_PROGRAM = new PublicKey("4cTXXUpQHbzrLRgXxqkFKFe8oHHjivUzaHP7d78EYTGK");
const App: React.FC = () => {
    ...
    const getProgramClient = useCallback(async (programId: PublicKey): Promise<Program> => {
        let idl: any = await Program.fetchIdl(programId, provider.current);
        if (!idl) {
            // Fallback to bundled IDL if on-chain IDL is missing
            try {
                const res = await fetch('/idl/simple_agent.json');
                if (res.ok) {
                    idl = await res.json();
                }
            } catch (_) {}
        ...
        // Hydrate accounts[].type from types[] if missing (IDL format differences)
        try {
            if (idl.accounts && idl.types) {
                const tmap: Record<string, any> = {};
                for (const t of idl.types) tmap[t.name] = t.type;
                let changed = false;
                idl.accounts = idl.accounts.map((acc: any) => {
                    if (!acc.type && tmap[acc.name]) {
                        changed = true;
                        return { ...acc, type: tmap[acc.name] };
                    }
                    return acc;
                });
                if (changed) {
            ...
            console.warn('IDL hydration skipped:', e);
        }
        // Use (idl, programId, provider). Cast to any to satisfy TS across versions.
        return new (Program as any)(idl, programId, provider.current as any);
    }, [provider]);
    ...
    const processLogs = useCallback(async (logs: Logs) => {
        let reply = null as string | null;
        let amount = null as bigint | null;
        logs.logs.forEach((log) => {
            if (log.includes("Agent Reply:") || log.includes("Agent Response:")) {
                reply = log
                    .replace("Agent Reply:", "")
                    .replace("Agent Response:", "")
                    .replace("Program log: ", "")
                    .trim()
                    .replace(/^"|"$/g, '');
            }
            if (log.includes("Amount:")) {
                try {
                    amount = BigInt(
                        log.replace("Amount:", "").replace("Program log: ", "").trim()
                    );
                } catch (error) {
                    amount = BigInt(0);
                }
        ...
        // Show reply even if no Amount is present (e.g., simple-agent)
        if (reply && prevSignature.current !== logs.signature) {
            prevSignature.current = logs.signature;
            console.log(`Agent response: ${reply}, amount: ${amount ?? BigInt(0)}, transaction: ${logs.signature}`);
            setIsLoading(false);
            setAgentReply([reply, amount ?? BigInt(0), logs.signature]);
            if ((amount ?? BigInt(0)) > BigInt(0)) {
    ...
    useEffect(() => {
        const initializeProgramClient = async () => {
            ...
            try {
                ...
                // Subscribe to logs for this wallet/context
                ...
                console.error('Program client init failed:', e);
                // Surface the exact error in the UI so users know what to do
                setTransactionError(String(e));
                setIsClientInitialized(false);
            }
        };
        initializeProgramClient().catch((e) => {
            console.error('Init unhandled error:', e);
            setTransactionError(String(e));
        });
    }, [connection, getProgramClient, getInteractionAddress, processLogs, publicKey]);
    ...
    const doInteractionTx = useCallback(async (inputText: string) => {
        if (!publicKey) throw new WalletNotConnectedError();
        if (!isClientInitialized || !agentProgramClient.current) {
            setTransactionError('Program client is not initialized yet. Connect wallet and wait a moment.');
            return;
        }
        if (!agent || !agent.current) {
            setTransactionError('Agent not found on-chain. Run Initialize in tests first.');
            return;
        }
