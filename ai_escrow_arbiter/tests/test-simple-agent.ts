...
describe("simple-agent", () => {
  ...
  // Ensure oracle PDAs and agent exist so tests can run standalone
  before(async () => {
    // Derive PDAs
    ...
    let counter;
    try {
      counter = await program_llm.account.counter.fetch(counterPda);
    } catch (_) {
      try {
        await program_llm.methods
          .initialize()
          .accounts({
    ...
    // Create next context PDA (uses current counter)
    const contextPda = web3.PublicKey.findProgramAddressSync(
      [Buffer.from("test-context"), new BN(counter.count).toArrayLike(Buffer, "le", 4)],
      program_llm.programId
    )[0];

    // Initialize agent if needed (also creates the context via CPI)
    try {
      ...
      try {
        ...
        // if another test created it concurrently, ignore
      }
    }
  });
