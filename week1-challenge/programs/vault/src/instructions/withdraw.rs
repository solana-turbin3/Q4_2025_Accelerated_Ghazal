...
impl<'info> Withdraw<'info> {
    pub fn handler<'c: 'info>(
        ...
        let whitelist_pda: Pubkey =
            Pubkey::find_program_address(whitelist_seeds, &WHITELIST_PROGRAM_ID).0;
        ...
        invoke_signed(&ix, &ctx_account_infos, &[vault_seeds])?;
...
mod tests {
    ...
    use crate::tests::cpi::{
        ...
        InitializeWhitelistAccounts, InitializeWhitelistData, WithdrawAccounts, WithdrawData,
    };
    ...
    fn withdraw_from_vault() {
        ...
        let whitelisted_address = vault_pda;
        let whitelist_pda = get_whitelist_pda(&whitelisted_address);

        println!("extra account meta list: {}", extra_account_meta_list_pda);
        println!("whitelist_pda: {}", whitelist_pda);
        ...
        let pre_depositor_ata_bal = fetch_account::<TokenAccount>(litesvm, &depositor_ata).amount;
        let pre_vault_ata_bal = fetch_account::<TokenAccount>(litesvm, &vault_ata).amount;
        ...
        assert_eq!(pre_depositor_ata_bal, post_depositor_ata_bal - amount);

        let post_vault_ata_bal = fetch_account::<TokenAccount>(litesvm, &vault_ata).amount;

        assert_eq!(pre_vault_ata_bal, post_vault_ata_bal + amount);
    }
}
