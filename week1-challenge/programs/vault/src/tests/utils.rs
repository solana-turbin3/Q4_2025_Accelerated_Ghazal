...
use crate::tests::constants::{PROGRAM_ID, WHITELIST_PROGRAM_ID};

pub fn setup() -> (LiteSVM, Keypair) {
    let mut litesvm = LiteSVM::new();
    litesvm
        .add_program_from_file(PROGRAM_ID.to_bytes(), "../../target/deploy/vault.so")
        .unwrap();
    litesvm
        .add_program_from_file(
            WHITELIST_PROGRAM_ID.to_bytes(),
            "../../target/deploy/whitelist.so",
        )
        .unwrap();
